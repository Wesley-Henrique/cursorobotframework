*** Settings ***
Documentation        Aqui estatão presentes todas as Keywords responsáveis pelo testes de API.
...
${BODY_BOOK}
...        id=202
...        title="Apredendo Robot"
...        description=Testando API de cadastro
...        pageCount=5464
...        excerpt=bla bla bla
...
E o body com todas as informações enviadas no cadastro
    Dictionary Should Contain Item    ${RESPOSTA.json()}    id             ${BODY_BOOK.id}
    Dictionary Should Contain Item    ${RESPOSTA.json()}    title          ${BODY_BOOK.title}
    Dictionary Should Contain Item    ${RESPOSTA.json()}    description    ${BODY_BOOK.description}
    Dictionary Should Contain Item    ${RESPOSTA.json()}    pageCount      ${BODY_BOOK.pageCount}
    Dictionary Should Contain Item    ${RESPOSTA.json()}    excerpt        ${BODY_BOOK.excerpt}
    Dictionary Should Contain Item    ${RESPOSTA.json()}    publishDate    ${BODY_BOOK.publishDate}
